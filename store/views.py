...
def shop(request):
    ...
    prices = [
        {"id": "lowest", "value": "От высокой цены к низкой"},
        {"id": "highest", "value": "От низкой цены к высокой"},
    ]
...
def add_to_cart(request):
    ...
    if not id or not qty or not cart_id:
        return JsonResponse({"error": "Выберите цвет и размер"}, status=400)
    ...
    if int(qty) > product.stock:
        return JsonResponse({"error": "Запрашиваемое количество отсутствует"}, status=404)
...
def create_order(request):
    if request.method == "POST":
        ...
        if not address_id:
            messages.warning(request, "Выберите адрес, чтобы продолжить")
...
def contact(request):
    if request.method == "POST":
        ...
        messages.success(request, "Ваше сообщение отправлено в творческую вселенную!")
